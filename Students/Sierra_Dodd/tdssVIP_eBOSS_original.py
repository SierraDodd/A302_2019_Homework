...
#            PS1file = urllib.urlretrieve('http://students.washington.edu/jruan/tdssVIP/PS3pi_lightcurves/eBOSS/'+filename)
...
class callback_handler:
    ...
    def next(self):
        ...
        ax2.errorbar(currentplate.wavelength, smooth(currentplate.spectra[TDSS_fiber_index]), yerr=smooth(currentplate.fluxerr[TDSS_fiber_index]), ecolor='g', color='k', capsize=0.)
    ...
    def restframe(self, event):
        ...
        if (self.specframe == 0):
            ...
            ax2.set_xlabel(r'wavelength [$\AA$]', size=12)
            ax2.set_title('fiber = '+str(TDSS_fiber_index+1)+', class = '+currentplate.class0[TDSS_fiber_index]+', subclass = '+currentplate.subclass0[TDSS_fiber_index]+', z = '+str(round(currentplate.redshift[TDSS_fiber_index], 3)), fontsize=14)
            ax2.errorbar(currentplate.wavelength/(1.+currentplate.redshift[TDSS_fiber_index]), smooth(currentplate.spectra[TDSS_fiber_index]), yerr=smooth(currentplate.fluxerr[TDSS_fiber_index]), ecolor='g', color='k', capsize=0.)
